...
class hashtable:
	...
	def __init__(self):
		self.T=[None for i in range(0,351)];
	...
	def inserthash(self,string):
		...
		if(self.T[v]==None):
			...
			self.T[v].insert(string,self.T[v].head,v)
	def display(self):
		print("hash table display starts")
	...
	def searchhash(self,string):
		...
		if self.T[v]==None:
			...
			if addr==None:
				...
				print("at",addr,"hash table index is",v,"valid")
	def keyhash(self):
		for i in range(0,len(self.T)):
			if self.T[i]==None:
				print("None",end=" ")
...
def main():
    ...
    #a.display()
    c=True
    print("enter elements to be searched or enter -1 to exit")
    while c:
        s=int(input())
        if s==-1:
            c=False
